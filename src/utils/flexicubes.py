...
from .flexicubes_tables import check_table, dmc_table, num_vd_table, tet_table


class FlexiCubes(nn.Module):
    """
    ...
    def __init__(self, qef_reg_scale=1e-3, weight_scale=0.99):
        ...
        self.register_buffer(
            "dmc_table",
            torch.tensor(dmc_table, dtype=torch.long, requires_grad=False),
            persistent=False,
        )
        self.register_buffer(
            "num_vd_table",
            torch.tensor(num_vd_table, dtype=torch.long, requires_grad=False),
            persistent=False,
        )
        self.register_buffer(
            "check_table",
            torch.tensor(check_table, dtype=torch.long, requires_grad=False),
            persistent=False,
        )
        self.register_buffer(
            "tet_table",
            torch.tensor(tet_table, dtype=torch.long, requires_grad=False),
            persistent=False,
        )
        self.register_buffer(
            "quad_split_1",
            torch.tensor([0, 1, 2, 0, 2, 3], dtype=torch.long, requires_grad=False),
            persistent=False,
        )
        self.register_buffer(
            "quad_split_2",
            torch.tensor([0, 1, 3, 3, 1, 2], dtype=torch.long, requires_grad=False),
            persistent=False,
        )
        self.register_buffer(
            "quad_split_train",
            torch.tensor(
                [0, 1, 1, 2, 2, 3, 3, 0], dtype=torch.long, requires_grad=False
            ),
            persistent=False,
        )
        self.register_buffer(
        ...
        self.register_buffer(
            "cube_corners_idx",
            torch.pow(2, torch.arange(8, requires_grad=False)),
            persistent=False,
        )
        self.register_buffer(
            "cube_edges",
    ...
    def construct_voxel_grid(self, res):
        ...
        """
        base_cube_f = torch.arange(8).to(self.dmc_table.device)
        if isinstance(res, int):
            res = (res, res, res)
        voxel_grid_template = torch.ones(res, device=self.dmc_table.device)

        res = torch.tensor([res], dtype=torch.float, device=self.dmc_table.device)
        ...
        cubes = (
            ...
            + torch.arange(coords.shape[0], device=self.dmc_table.device).unsqueeze(1)
            * 8
        ).reshape(-1)
    ...
    def __call__(
        ...
        if surf_cubes.sum() == 0:
            return (
                torch.zeros((0, 3), device=self.dmc_table.device),
                torch.zeros((0, 4), dtype=torch.long, device=self.dmc_table.device)
                if output_tetmesh
                else torch.zeros(
                    (0, 3), dtype=torch.long, device=self.dmc_table.device
                ),
                torch.zeros((0), device=self.dmc_table.device),
            )
    ...
    def _normalize_weights(self, beta_fx12, alpha_fx8, gamma_f, surf_cubes):
        ...
        if beta_fx12 is not None:
            ...
            beta_fx12 = torch.ones(
                (n_cubes, 12), dtype=torch.float, device=surf_cubes.device
            )

        if alpha_fx8 is not None:
            alpha_fx8 = torch.tanh(alpha_fx8) * self.weight_scale + 1
        else:
            alpha_fx8 = torch.ones(
                (n_cubes, 8), dtype=torch.float, device=surf_cubes.device
            )
        ...
        if gamma_f is not None:
            ...
            gamma_f = torch.ones((n_cubes), dtype=torch.float, device=surf_cubes.device)
    ...
    def _get_case_id(self, occ_fx8, surf_cubes, res):
        ...
        case_ids = (
            occ_fx8[surf_cubes]
            * self.cube_corners_idx.to(self.dmc_table.device).unsqueeze(0)
        ).sum(-1)

        problem_config = self.check_table.to(self.dmc_table.device)[case_ids]
        ...
        problem_config_full = torch.zeros(
            list(res) + [5], device=self.dmc_table.device, dtype=torch.long
        )
        ...
        idx = torch.arange(case_ids.shape[0], device=self.dmc_table.device)[to_check][
            within_range
        ][to_invert]
    ...
    def _linear_interp(self, edges_weight, edges_x):
        ...
        edges_weight = torch.cat(
            [
                torch.index_select(
                    input=edges_weight,
                    index=torch.tensor(1, device=self.dmc_table.device),
                    dim=edge_dim,
                ),
                -torch.index_select(
                    input=edges_weight,
                    index=torch.tensor(0, device=self.dmc_table.device),
                    dim=edge_dim,
                ),
            ],
            edge_dim,
    ...
    def _compute_vd(
        ...
        for num in torch.unique(num_vd):
            ...
            curr_edge_group_to_vd = (
                torch.arange(curr_num_vd, device=self.dmc_table.device)
                .unsqueeze(-1)
                .repeat(1, 7)
                + total_num_vd
            )
            ...
            curr_edge_group_to_cube = (
                torch.arange(idx_map.shape[0], device=self.dmc_table.device)[cur_cubes]
                .unsqueeze(-1)
                .repeat(1, num * 7)
        ...
        if grad_func is not None:
            vd = torch.cat(vd)
            L_dev = torch.zeros([1], device=self.dmc_table.device)
        else:
            vd = torch.zeros((total_num_vd, 3), device=self.dmc_table.device)
            beta_sum = torch.zeros((total_num_vd, 1), device=self.dmc_table.device)
        ...
        v_idx = torch.arange(
            vd.shape[0], device=self.dmc_table.device
        )  # + total_num_vd
    ...
    def _triangulate(
        ...
        if grad_func is not None:
            ...
            gamma_02 = torch.index_select(
                ...
                input=quad_gamma,
                index=torch.tensor(2, device=self.dmc_table.device),
                dim=1,
            )
            gamma_13 = torch.index_select(
                ...
                input=quad_gamma,
                index=torch.tensor(3, device=self.dmc_table.device),
                dim=1,
            )
        if not training:
            ...
            vd_center_idx = (
                torch.arange(vd_center.shape[0], device=self.dmc_table.device)
                + vd.shape[0]
            )
    ...
    def _tetrahedralize(
        ...
        mapping_inside_verts = (
            torch.ones((occ_n.shape[0]), dtype=torch.long, device=self.dmc_table.device)
            * -1
        )
        mapping_inside_verts[occ_n] = (
            torch.arange(occ_n.sum(), device=self.dmc_table.device) + vertices.shape[0]
        )
        """
        ...
        mapping = (
            torch.ones(
                (unique_edges.shape[0]), dtype=torch.long, device=self.dmc_table.device
            )
            * -1
        )
        mapping[mask_edges] = torch.arange(
            mask_edges.sum(), device=self.dmc_table.device
        )
        idx_map = mapping[_idx_map]
        ...
        edge_idx = (
            torch.arange((12), dtype=torch.long, device=self.dmc_table.device)
            .unsqueeze(0)
