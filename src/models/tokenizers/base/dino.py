...
from src.models.transformers.attention import MemoryEfficientAttentionMixin
from src.utils.typing import Callable, Dict, List, Optional, Set, Tuple, Union
...
class ViTLayer(nn.Module, MemoryEfficientAttentionMixin):
    ...
    def __init__(self, config: ViTConfig) -> None:
        ...
        self.layernorm_before_modulation = None
        self.layernorm_after_modulation = None
    ...
    def forward(
        ...
        if self.layernorm_before_modulation is not None:
            assert modulation_cond is not None
            hidden_states_norm = self.layernorm_before_modulation(
        ...
        if self.layernorm_after_modulation is not None:
            assert modulation_cond is not None
            layer_output = self.layernorm_after_modulation(
                layer_output, modulation_cond
            )
    ...
    def register_ada_norm_modulation(self, norm1_mod: nn.Module, norm2_mod: nn.Module):
        self.layernorm_before_modulation = norm1_mod
        self.layernorm_after_modulation = norm2_mod


class ViTEncoder(nn.Module, MemoryEfficientAttentionMixin):
    def __init__(self, config: ViTConfig) -> None:
        super().__init__()
        self.config = config
...
class ViTModel(ViTPreTrainedModel, MemoryEfficientAttentionMixin):
    def __init__(
        self,
        config: ViTConfig,
