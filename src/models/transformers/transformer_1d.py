...
from src.models.transformers.attention import (
...
class Transformer1D(BaseModule, MemoryEfficientAttentionMixin):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.norm_num_groups > 0:
            ...
            self.norm = nn.LayerNorm(self.cfg.in_channels)
    ...
    def forward(
        ...
        timestep = outputs.get(Names.TIMESTEPS, None)
        ...
        batch_dimensions_hidden_states = hidden_states.shape[:-2]
        batch_dimensions_encoder_hidden_states = encoder_hidden_states.shape[:-2]

        if (
            len(batch_dimensions_hidden_states) > 1
            or len(batch_dimensions_encoder_hidden_states) > 1
        ):
        ...
        return {self.cfg.output_key: output}
    ...
    def produced_keys(self):
        return super().produced_keys().union({self.cfg.output_key})
